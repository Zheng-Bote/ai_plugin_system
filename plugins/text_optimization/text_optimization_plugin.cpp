...
public:
    [[nodiscard]] Task<std::expected<std::string, std::string>> analyze(std::string input_json, LLMClient* llm_client) override {
        ...
        LLMQuery q;
        q.system_prompt = std::string("Optimiere den Text. JSON Output.") + get_schema_instruction();
    ...
    [[nodiscard]] Generator<std::string> analyze_stream(std::string input_json, LLMClient* llm_client) override {
        ...
        const auto& text = input_pair.first;
        [[maybe_unused]] const auto& mode = input_pair.second;
        LLMQuery q; q.system_prompt = std::string("Optimiere Text. JSON Stream.") + get_schema_instruction(); q.prompt = text;
    ...
    [[nodiscard]] std::string_view get_description() const override { return "Optimizes text for clarity, tone, and grammar."; }
