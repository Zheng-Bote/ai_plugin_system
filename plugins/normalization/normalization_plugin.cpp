/**
 * SPDX-FileComment: Normalization Plugin Implementation (Refactored)
 ...
 * @brief Production-ready text normalization inheriting from BasePlugin
 * @version 0.4.2
 * @date 2026-04-05
 *
...
namespace ai_plugin {
class NormalizationPlugin : public BasePlugin {
public:
    [[nodiscard]] Task<std::expected<std::string, std::string>> analyze(std::string input_json, LLMClient* llm_client) override {
        ...
        LLMQuery q; q.system_prompt = std::string("Normalisiere Text. JSON Output.") + get_schema_instruction(); q.prompt = text; q.json_schema = m_schema.dump();
    ...
    [[nodiscard]] Generator<std::string> analyze_stream(std::string input_json, LLMClient* llm_client) override {
        ...
        const auto& text = input_pair.first;
        [[maybe_unused]] const auto& mode = input_pair.second;
        LLMQuery q; q.system_prompt = std::string("Normalize.") + get_schema_instruction(); q.prompt = text;
    ...
    [[nodiscard]] std::string_view get_description() const override { return "Normalizes text into a consistent format."; }
protected:
    [[nodiscard]] std::string get_schema_path() const override { return "data/schemas/normalization.schema.json"; }
};
extern "C" { Plugin* create_plugin() { return new NormalizationPlugin(); } void destroy_plugin(Plugin* p) { delete p; } }
}
