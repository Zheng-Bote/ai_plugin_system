/**
 * SPDX-FileComment: Keyword Extraction Plugin Implementation
 ...
 * @brief Production-ready keyword, topics, and entities extraction
 * @version 0.1.0
 * @date 2026-04-05
 *
...
class KeywordsPlugin : public Plugin {
public:
    [[nodiscard]] std::expected<void, std::string> init(std::string_view config_json) override {
        try {
            ...
            std::ifstream schema_file("data/schemas/keywords.schema.json");
            if (schema_file.is_open()) {
                m_schema = json::parse(schema_file);
            }
        } catch (const std::exception& e) {
            return std::unexpected(std::format("KeywordsPlugin init fail: {}", e.what()));
        }
        return {};
    }
    ...
    [[nodiscard]] std::expected<std::string, std::string> analyze(std::string_view input_json, LLMClient* llm_client) override {
        ...
        LLMQuery query;
        query.system_prompt = "Extrahiere Themen, Entitäten und Tags aus dem Text. Antworte in striktem JSON gemäß Schema.";
        query.prompt = text;
    ...
    [[nodiscard]] Generator<std::string> analyze_stream(std::string_view input_json, LLMClient* llm_client) override {
        ...
        std::string text = parse_input(input_json);
        LLMQuery query;
        query.system_prompt = "Extrahiere Keywords (Topics, Entities, Tags). Antworte als Stream.";
        query.prompt = text;
        query.task_type = TaskType::ANALYSIS;
...
extern "C" {
    Plugin* create_plugin() { return new KeywordsPlugin(); }
